...
import { DeXuatNguoiHienMauService } from 'src/app/shared/services/de-xuat-nguoi-hien-mau.service';
...
@Component({
  selector: 'app-de-xuat-ho-gia-dinh',
  templateUrl: './de-xuat-ho-gia-dinh.component.html',
  styleUrls: ['./de-xuat-ho-gia-dinh.component.scss']
})
export class DeXuatHoGiaDinhComponent implements OnInit {
  ...
  loadDottonvinh(){
    ...
    this.getDottonvinh().subscribe((item:any)=>{
      setTimeout(()=>{
  ...
  loadDonvi(){
    this.isSpinning = true;
    this.getDonvi().subscribe((item:any)=>{
    setTimeout(()=>{
        this.donvis = item;
        this.isSpinning = false;
      }, 1000)
